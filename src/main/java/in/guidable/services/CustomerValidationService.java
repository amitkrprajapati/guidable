...
public class CustomerValidationService {
    ...
    public static final String VALIDATION_VERIFY_REGISTRATION_TOKEN = "/validation/verifyRegistration?token=";
    ...
    public void checkIfCustomerEnabled(AuthRequest authRequest) {
        boolean isCustomerEnabled = customerRepo.findByCustomerUserName(authRequest.getUserName())
                .get().isEnabled();
    ...
    public void saveVerificationTokenForUser(String token, Customer customer) {
        VerificationToken verificationToken
                = new VerificationToken(customer, token);
    ...
    public String resendVerificationToken(String userName, String prefixUrl) {
        ...
        VerificationToken oldVerificationToken = verificationTokenRepository.
                getVerificationTokenByUserId(customer.getId());
        ...
        if(oldVerificationToken!=null){
            ...
            Calendar cal = Calendar.getInstance();
            if ((oldVerificationToken.getExpirationTime().getTime()
                    - cal.getTime().getTime()) <= 0) {
            ...
            String url =
                            prefixUrl
                            + VALIDATION_VERIFY_REGISTRATION_TOKEN
                            + newToken;
            log.info("Click the link to verify your account new URL: {}",
                    url);
        }
        return "Email Sent";
    }
